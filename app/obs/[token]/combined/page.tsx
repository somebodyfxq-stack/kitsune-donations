...
import { CombinedWidgetClient } from "../../combined-widget-client";
...
export default async function CombinedPage({ params, searchParams }: CombinedPageProps) {
  ...
  console.log("🔥 Combined Widget - Looking for user with token:", token);
  ...
  try {
    ...
    if (!user) {
      ...
      return (
        <html lang="uk">
          <head><title>Помилка - Combined Віджет</title></head>
          <body style={{ padding: '20px', fontFamily: 'monospace', background: '#000', color: '#fff' }}>
            <h1>🔍 Debug Info - Combined Widget</h1>
    ...
    return (
      <html lang="uk">
        <head><title>Database Error - Combined Віджет</title></head>
        <body style={{ padding: '20px', fontFamily: 'monospace', background: '#000', color: '#fff' }}>
          <h1>💥 Database Error - Combined Widget</h1>
          <p><strong>Token from URL:</strong> {token}</p>
  ...
  return (
    <html lang="uk">
      <head>
        <title>Комбінований Віджет - Kitsune Donations</title>
